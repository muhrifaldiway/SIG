<x-layout>
    <x-slot:title>Tambah Sebaran</x-slot>

    <h1 class="text-3xl font-bold mb-4">Tambah Sebaran</h1>
    ...
    <form action="{{ route('sebaran.store') }}" method="POST" class="flex">
        @csrf
        ...
        <div class="w-1/2 pr-4">
            <div class="mb-4">
                ...
                <select name="satwa_id" id="satwa_id" class="block w-full mt-1 py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    ...
                    @foreach ($satwa as $s)
                        <option value="{{ $s->id }}">{{ $s->nama_umum }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="kawasan_id" id="kawasan_id" class="block w-full mt-1 py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    ...
                    @foreach ($kawasan as $k)
                        <option value="{{ $k->id }}">{{ $k->nama_kawasan }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="date" name="tanggal" id="tanggal" class="block w-full mt-1 py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
            </div>

            <div class="mb-4">
                <label for="jumlah" class="block text-sm font-medium text-gray-700">Jumlah</label>
                <input type="number" name="jumlah" id="jumlah" class="block w-full mt-1 py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
            </div>

            <div class="mb-4">
                <label for="keterangan" class="block text-sm font-medium text-gray-700">Keterangan</label>
                <textarea name="keterangan" id="keterangan" rows="3" class="block w-full mt-1 py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"></textarea>
            </div>

            <div class="mb-4">
                <label for="latitude" class="block text-sm font-medium text-gray-700">Latitude</label>
                <input type="text" name="latitude" id="latitude" class="block w-full mt-1 py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
            </div>

            <div class="mb-4">
                <label for="longitude" class="block text-sm font-medium text-gray-700">Longitude</label>
                <input type="text" name="longitude" id="longitude" class="block w-full mt-1 py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            var map = L.map('map').setView([-1.443379188614197, 119.9861757299177], 8); // Set zoom level to 8
            ...
            if (latitudeField.value && longitudeField.value) {
                marker = L.marker([latitudeField.value, longitudeField.value]).addTo(map);
            }
            ...
            // Event listener untuk menangani hasil pencarian
            searchControl.getProvider().on('results', function (data) {
                map.setView(data.latlng, 16); // Atur zoom level yang diinginkan di sini
            });
        });
    </script>
</x-layout>
